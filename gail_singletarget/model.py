...
class RnnNet_DLGN(nn.Module):
    def __init__(self, config, vocab, device):
        ...
        self.gru = nn.GRU(self.embed_dim, self.blstm_dim, num_layers=self.num_layers,
                          ...
                          batch_first=True)
        self.hidden2out = nn.Sequential(nn.Linear(self.num_layers * self.num_dir * self.blstm_dim, self.blstm_dim),
                                        nn.Sigmoid(),
                                        nn.Dropout(p=0.3))
        self.double_out = nn.Linear(2 * self.hidden_size, self.out_dim)
    ...
    def forward(self, inp1, inp2):
        ...
        logits = torch.cat([x1_out, x2_out], dim=1)
        logits = self.double_out(logits)
        # probs = F.softmax(logits, dim=-1)

        return logits

    def forward_v1(self, inp1, inp2, inp3):
        '''
...
class Discriminator:
    ...
    def compute_reward_gru(self, input1, gen_input):
        out = self.model.forward(input1, gen_input)
