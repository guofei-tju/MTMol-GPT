...
from toolkits.scripts import sascorer
from toolkits.utils import canonical_smiles, canonical
...
Chembl24_sanitized, _ = canonical_smiles(Chembl24_smiles.values.reshape(-1), sanitize=True, throw_warning=False)
...
target1_sanitized, _ = canonical_smiles(target1_smiles.values.reshape(-1), sanitize=True, throw_warning=False)
...
target2_sanitized, _ = canonical_smiles(target2_smiles.values.reshape(-1), sanitize=True, throw_warning=False)
...
sm_smiles_path = '../multitarget_gail/results/chembl_pretrain/generation/drd2_htr1a/2022-09-15/14-06-05(decision_sm_results)/'
sm_save_path = '../multitarget_gail/results/chembl_pretrain/generation/drd2_htr1a/2022-09-15/14-06-05(decision_sm_results)/plot/'

sf_iter = 190
sf_file_name = f'finetune_gail_num30000_e{sf_iter}'
sf_smiles_path = '../multitarget_gail/results/chembl_pretrain/generation/drd2_htr1a/2022-09-16/09-10-02(decision_sf_results)/'
sf_save_path = '../multitarget_gail/results/chembl_pretrain/generation/drd2_htr1a/2022-09-16/09-10-02(decision_sf_results)/plot/'
...
## sm
# pretrain smiles
pre_SMILES = pd.read_csv(sm_smiles_path + 'finetune_gail_num30000_e0' + '.txt', names=['smiles'],
                         header=None)
_, _, pre_sanitized = canonical(pre_SMILES.values.reshape(-1), sanitize=True, throw_warning=False)
unique_pre_sm_smiles = list(np.unique(pre_sanitized))
# random
unique_pre_sm_indices = np.arange(len(unique_pre_sm_smiles))
np.random.shuffle(unique_pre_sm_indices)
unique_pre_sm_smiles = itemgetter(*unique_pre_sm_indices[:10000])(unique_pre_sm_smiles)

# trained smiles
SMILES = pd.read_csv(sm_smiles_path + sm_file_name + '.txt', names=['smiles'],
                     header=None)
_, _, sanitized = canonical(SMILES.values.reshape(-1), sanitize=True, throw_warning=False)
...
## sf
# pretrain selfies
pre_SMILES = pd.read_csv(sf_smiles_path + 'finetune_gail_num30000_e0' + '.txt', names=['smiles'],
                         header=None)
_, _, pre_sanitized = canonical(pre_SMILES.values.reshape(-1), sanitize=True, throw_warning=False)
unique_pre_sf_smiles = list(np.unique(pre_sanitized))
# random
unique_pre_sf_indices = np.arange(len(unique_pre_sf_smiles))
np.random.shuffle(unique_pre_sf_indices)
unique_pre_sf_smiles = itemgetter(*unique_pre_sf_indices[:10000])(unique_pre_sf_smiles)
...
# #iterative the generated SMILES
# for iter in trange(0, 2000, 50):
#     file_name = f'generation30000itertaion{iter}_ALLlayer_SMILES'
#     SMILES = pd.read_csv(smiles_path + file_name + '.csv')
#     sanitized, _ = canonical_smiles(SMILES.values.reshape(-1), sanitize=True, throw_warning=False)
#     unique_smiles = list(np.unique(sanitized))
#
#     # random
#     indices = np.arange(len(unique_smiles))
#     np.random.shuffle(indices)
#
#     SMILES_logP = Estimate_logP(itemgetter(*indices[:10000])(unique_smiles))
#     # SMILES_logPs.append(SMILES_logP)
#     color = age_colors[iter % len(age_colors)]
#     plot_hist(SMILES_logP, save_path, color, iter)
